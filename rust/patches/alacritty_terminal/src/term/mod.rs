...
impl<T> Term<T> {
    ...
    /// Resize the terminal without reflowing content.
    ///
    /// Updates all internal state (scroll region, tabs, damage, inactive grid)
    /// but rows are truncated or padded instead of being rewrapped. This avoids
    /// lossy reflow of content with exact-width filler characters (e.g. powerline prompts).
    pub fn resize_no_reflow<S: Dimensions>(&mut self, size: S) {
        ...
        // Resize both grids without reflowing content.
