...
- [beads](https://github.com/hex/beads) (`bd` CLI) for issue tracking sidebar
...
The Rust library is built automatically by a pre-build Run Script phase. Dev builds use `opt-level = 2` for acceptable VTE throughput.

### Release

```bash
CONFIGURATION=Release xcodebuild -scheme Claide -destination 'platform=macOS' \
  ONLY_ACTIVE_ARCH=YES build
```

`ONLY_ACTIVE_ARCH=YES` is required because `rust/build-universal.sh` builds only the native architecture. Release builds enable thin LTO and `codegen-units = 1`.

A release script is available at `scripts/release.sh`. CI (GitHub Actions on macOS-15) handles signing with Developer ID, notarization, DMG creation, and GitHub Release publishing.

### Dependencies

No Swift Package Manager dependencies — all system frameworks (AppKit, Metal, MetalKit, CoreText, QuartzCore). Rust dependencies are vendored through Cargo.

## Project Structure

```
Claide/
  ClaideApp.swift                # Entry point, NSApplicationDelegateAdaptor
...
rust/
  ...
  build-universal.sh             # Builds native-arch static library
ClaideTests/                     # 153 tests across 20 test files
project.yml                      # XcodeGen spec
scripts/
  release.sh                     # Local release build
tools/
  termbench.sh                   # Terminal throughput benchmark
.github/
  workflows/release.yml          # CI: sign, notarize, DMG, GitHub Release
```
